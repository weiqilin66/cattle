...
public class InitPubDbService {
    ...
    public void init() {
        if (ObjectUtils.isEmpty(this.initDatasource)) {
            MysqlDbUtilsQ.initDb(this.url, this.password, this.username, this.dbId, this.maxActive, this.maxWait, this.minIdle);
        } else {
            MysqlDbUtilsQ.addDb(this.dbId, this.initDatasource);
        }

        JdbcTemplate jdbcTemplate = MysqlDbUtilsQ.getTemplate(this.dbId);
        ...
        while (var4.hasNext()) {
            ...
            MysqlDbUtilsQ.initDb(db_url, db_password, db_username, db_dbId);
        }
